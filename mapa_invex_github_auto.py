...
def extraer_determinante(valor):
    """
    Devuelve una llave limpia para unir Base / Records / Asistencia.
    ...
    La llave queda SIN ceros a la izquierda para que coincidan
    formatos diferentes del mismo determinante.
    """
    if pd.isna(valor):
        return ""
    ...
    # Caso especial: determinantes escritos como 00-20, 00/20, 0-179, etc.
    match_guion = re.search(r"(?<!\d)(\d{1,3})\s*[-/]\s*(\d{1,3})(?!\d)", texto)
    if match_guion:
        digitos = match_guion.group(1) + match_guion.group(2)
    else:
        # Buscar grupos numéricos; prioriza códigos de 2 a 5 dígitos.
        grupos = re.findall(r"\d+", texto)
        if not grupos:
            return ""
        candidatos = [g for g in grupos if 2 <= len(g) <= 5]
        digitos = candidatos[0] if candidatos else grupos[0]

    digitos = re.sub(r"\D", "", str(digitos))
    if not digitos:
        return ""

    # Normalizar quitando ceros a la izquierda para los cruces.
    key = digitos.lstrip("0")
    return key if key else "0"

def extraer_determinante_mostrar(valor):
    """
    Devuelve el determinante para mostrar en el mapa, conservando formatos
    útiles como 00-20, 0013, 0179 cuando vienen así en BASE DE TIENDAS.
    """
    if pd.isna(valor):
        return ""
    ...
    match_guion = re.search(r"(?<!\d)(\d{1,3})\s*[-/]\s*(\d{1,3})(?!\d)", texto)
    if match_guion:
        return f"{match_guion.group(1)}-{match_guion.group(2)}"
...
def crear_html_mapa(data_tiendas, salida_html, ultima_actualizacion=None):
    ultima_actualizacion = ultima_actualizacion or datetime.now().strftime('%d/%m/%Y %H:%M:%S')
...
<div class="panel" id="mainPanel">
    ...
    <div class="panel-body">
        ...
        <div class="update-box">🕒 Última actualización: {ultima_actualizacion}</div>
...
function agregarMarker(t) {{
    ...
    markerByDet[t.detKey || t.det] = marker;
    markerByDet[t.det] = marker;
}}

function setFiltroSemaforo(valor) {{
    filtroSemaforo = valor;
    document.querySelectorAll(".btn-filter").forEach(btn => {{
        btn.classList.toggle("active", btn.getAttribute("data-filtro") === valor);
    }});
    aplicarFiltros();
}}
...
function renderListaTiendas(lista) {{
    ...
    const ordenadas = [...lista].sort((a, b) => {{
        ...
        return String(a.detKey || a.det).localeCompare(String(b.detKey || b.det), "es", {{numeric:true}});
    }});
    ...
    mostrar.forEach(t => {{
        ...
        `;
        div.onclick = () => enfocarTienda(t.detKey || t.det);
...
function enfocarTienda(det) {{
    const t = tiendas.find(x => String(x.det) === String(det) || String(x.detKey || "") === String(det));
    if (!t) return;
    mostrarInfo(t);
...
function buscarTienda() {{
    ...
    const exacta = tiendas.find(t => (String(t.det) === texto || String(t.detKey || "") === texto) && (region === "TODAS" || t.region === region));
    if (exacta) {{ aplicarFiltros(); enfocarTienda(exacta.detKey || exacta.det); return; }}

    const parecida = tiendas.find(t => (String(t.det).includes(texto) || String(t.detKey || "").includes(texto)) && (region === "TODAS" || t.region === region));
    if (parecida) {{ aplicarFiltros(); enfocarTienda(parecida.detKey || parecida.det); return; }}
...
def main():
    ...
    if SOLO_TIENDAS_OPERANDO and col_estatus_base:
        # Solo tiendas realmente OPERANDO. Excluye BAJA y PENDIENTE aunque vengan con texto extra.
        base = base[
            base["ESTATUS_TIENDA_N"].str.contains("OPERANDO", na=False)
            & ~base["ESTATUS_TIENDA_N"].str.contains("BAJA|PENDIENTE", na=False)
        ].copy()

    print(f"Tiendas OPERANDO detectadas en base: {base['DET'].nunique()}")

    for det_revision in ["20", "13", "179"]:
        if det_revision not in set(base["DET"].astype(str)):
    ...
    for _, row in resumen.iterrows():
        ...
        data_tiendas.append({
            "det": str(row.get("DET_MOSTRAR", row["DET"])),
            "detKey": str(row["DET"]),
            "region": str(row["REGION_N"]),
    ...
    ultima_actualizacion = fecha_hoy_mexico().strftime("%d/%m/%Y %H:%M")
    crear_html_mapa(data_tiendas, salida_html, ultima_actualizacion=ultima_actualizacion)
